...
function httpRequest(callback) {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4) {
			//alert(xhr.responseText.split(";")[len-1]);
			if(xhr.responseText.split(";")[len-1].length<100){
				warning_1.style.display="block";
				var temp=localStorage.stocks.split(",");
				temp.pop();
				localStorage.stocks=temp;
			}
			else{
...
document.getElementById('addBtn').onclick = function() {
	var add = document.getElementById('add'),
		...
		warning_1=document.getElementById("warning_1");

	warning_1.style.display="none";
	var input = document.createElement('input');
	input.type = 'text';
	input.id = 'newStock';
	input.addEventListener("change",function(e){
		numberOnly.call(e.target,6);
		e.stopPropagation();
	});
	...
	document.getElementById('saveBtn').onclick = function() {
		...
		warning_2.style.display="none";

		switch (newStock.slice(0,1)){
			case "6":newStock="sh"+newStock;break;
			case "0":newStock="sz"+newStock;break;
			case "3":newStock="sz"+newStock;break;
			default :newStock="";warning_2.style.display="inline";return;
		}
